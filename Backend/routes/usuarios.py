...
def crear_usuario():
    try:
        ...
        """
        cursor.execute(query, (nombres, apellidos, correo, telefono, usuario, contrasena, rol))
        conn.commit()
    ...
    try:
        ...
        if contrasena:
            query = """
                ...
                WHERE id_usuario = %s
            """
            params = (nombres, apellidos, correo, telefono, usuario, contrasena, rol, usuario_id)
        else:
            query = """
                UPDATE usuarios
                SET nombres = %s, apellidos = %s, correo = %s, telefono = %s,
                ...
                WHERE id_usuario = %s
            """
            params = (nombres, apellidos, correo, telefono, usuario, rol, usuario_id)
